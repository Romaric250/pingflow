...
export class SpeedTester {
  private static readonly DEFAULT_OPTIONS: SpeedTestOptions = {
    timeout: 15000,
    retries: 2,
  ...
  private static readonly TEST_SERVERS: TestServer[] = [
    {
      name: 'Cloudflare',
   ...
   */
  static async runSpeedTest(options: Partial<SpeedTestOptions> = {}): Promise<SpeedTestResult> {
    ...
    for (const server of servers) {
      ...
      try {
        const result = await this.testDownloadSpeed(server, testOptions);
        if (result.success) {
   ...
   */
  private static async testDownloadSpeed(
    server: TestServer, 
      ...
      return new Promise((resolve, reject) => {
        ...
        const req = httpModule.get(server.downloadUrl, requestOptions, (res) => {
          ...
          const bar = new ProgressBar(
            `${chalk.cyan('Downloading')} [:bar] :percent :rateMB/s ETA: :etas`, 
            {
              total: totalSizeMB,
              width: 30,
   ...
   */
  static displayResults(result: SpeedTestResult): void {
