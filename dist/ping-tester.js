...
export class PingTester {
    ...
    static runPingTests() {
        return __awaiter(this, arguments, void 0, function* (options = {}) {
            ...
            for (const { name, host } of this.TEST_HOSTS) {
                ...
                try {
                    const result = yield this.pingHost(host, testOptions);
                    results.push(result);
                    if (result.alive) {
                        console.log(chalk.green(`✓ ${name}: ${(_a = result.avg) === null || _a === void 0 ? void 0 : _a.toFixed(1)}ms avg`));
                    }
                    else {
     ...
     */
    static parsePingOutput(host, output, platform) {
        const result = {
            host,
            alive: false
        };
     ...
     */
    static parseWindowsPingOutput(host, output) {
     ...
     */
    static parseUnixPingOutput(host, output) {
        const result = { host, alive: false };
        // Check if ping was successful
    ...
    static displayResults(results) {
        ...
        if (successfulPings.length > 0) {
            ...
            const avgLatencies = successfulPings.map(r => r.avg).filter(avg => avg !== undefined);
...
PingTester.DEFAULT_OPTIONS = {
    count: 4,
    timeout: 5000,
    interval: 1000
};
PingTester.TEST_HOSTS = [
